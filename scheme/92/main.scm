...
(define (base-eval exp env)
  (cond ((eof-object? exp) (d "eof") (cons env '*exit*))
	((constant? exp) (d "const") (cons env exp))
	((symbol? exp) (d "symb") (var-eval exp env))
	((not (pair? exp)) (d "non") (eval-error 'non-evaluatable exp env))
	((equal? (car exp) 'exit) (d "exit") (cons env '*exit*))
	((equal? (car exp) 'define) (d "def") (def-eval exp env))
	((equal? (car exp) 'let) (d "let") (let-eval exp env))
	((equal? (car exp) 'letrec) (d "letrec") (letrec-eval exp env))
	((equal? (car exp) 'lambda) (d "lambda") (lambda-eval exp env))
	((equal? (car exp) 'if) (d "if") (if-eval exp env))
	((equal? (car exp) 'begin) (d "begin") (begin-eval exp env))
	((equal? (car exp) 'quote) (d "quote") (quote-eval exp env))
	(else (d "else") (app-eval exp env))))
