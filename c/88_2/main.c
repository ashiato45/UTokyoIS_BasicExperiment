...
void process_input(dict *d, int ***graph, int *out_start, int *out_dest){
  ...
  while(scanf("%s %s %d", temp1, temp2, &dist) != EOF){
    ...
    if((*graph)[read1][read2] == -1 || (*graph)[read1][read2] > dist){
...
void dijkstra(dict *d, int ***graph, list **routes, int start, int dest){
  int i, j, k;
  list already;
  list nearest;
  int current = -1;
  int a = 0;
  push_list(start, &(*routes)[start]);
  init_list(&already);
  push_list(start, &already);
  for(k=0; k < d->size; k++){
    current = -1;
  for(i=0; i < d->size; i++){
    if(find_list(i, &already) == 1){
      for(j=0; j < d->size; j++){
	if(find_list(j, &already) == 0 && (*graph)[i][j] != -1){
	  a = calc_route(graph, &(*routes)[i]) + (*graph)[i][j];
	  if(current == -1 || a < current){
	    current = a;
	    copy_list(&nearest, &(*routes)[i]);
	    push_list(j, &nearest);
	  }
	}
      }
  ...
  if(current != -1){
    copy_list(&(*routes)[last_list(&nearest)], &nearest);
    push_list(last_list(&nearest), &already);
  }
...
void test4(dict *d, int ***graph, int start, int dest){
  dump_dict(d);
  dump_graph(d, graph);
  printf("start: %s, end: %s\n", d->table[start], d->table[dest]);
}
...
int main(){
  ...
  init_bigarray(&graph);
  routes = malloc(sizeof(list)*SIZE_TABLE);
  for(i=0; i < SIZE_TABLE; i++){
    init_list(&routes[i]);
  }
  ...
  process_input(&d, &graph, &start, &dest);
  #if DEBUG
  test4(&d, &graph, start, dest);
  #endif

  dijkstra(&d, &graph, &routes, start, dest);
  #if DEBUG
  for(i=0; i < d.size; i++){
    dump_list(&routes[i]);
    printf("%d\n", calc_route(&graph, &routes[i]));
  }
  #endif
  //  dump_list(&routes[dest]);
  //  dfs_nearpoint(&d, &graph, start, dest, &route);
  copy_list(&route, &routes[dest]);
  if(route.size > 1){
